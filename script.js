...
function syncGlobalProduct(productEl) {
    ...
    const profileCard = document.querySelector('.selected-dress-card');
    if (profileCard) {
        profileCard.setAttribute('data-color', color);
    }
...
document.addEventListener('click', (e) => {
    ...
    // 16. Apply Profile Button (and Card)
    const applyBtn = e.target.closest('#apply-profile-btn');
    const profileCard = e.target.closest('.selected-dress-card');
    ...
    // Capture & Apply Face Logic (MediaPipe Powered)
    if (e.target.closest('#capture-face-btn')) {
        const video = document.getElementById('webcam-video');
        if (!video || video.style.display === 'none' || !faceMesh) return;
        ...
        faceMesh.onResults((results) => {
            ...
            captureCanvas.width = 512; // Standard texture size
            ...
            if (results.multiFaceLandmarks && results.multiFaceLandmarks.length > 0) {
                ...
                // Add padding for realistic head mapping
                const width = maxX - minX;
                const height = maxY - minY;
                const centerX = minX + width / 2;
                const centerY = minY + height / 2;
                const size = Math.max(width, height) * 1.5; // Zoom out slightly to get full head

                const sX = (centerX - size / 2) * video.videoWidth;
                const sY = (centerY - size / 2) * video.videoHeight;
                const sSize = size * Math.max(video.videoWidth, video.videoHeight);

                // Draw Cropped Face (Centered & Mirror Correction)
                ctx.save();
                ctx.translate(captureCanvas.width, 0);
                ctx.scale(-1, 1);
                ctx.drawImage(video, sX, sY, sSize, sSize, 0, 0, captureCanvas.width, captureCanvas.height);
                ctx.restore();
            ...
            // Clean up listener to prevent memory leaks or multiple triggers
            faceMesh.onResults(() => {});
        });
