...
// Find green pixels (g > 35, g - r > 4, g - b > 4) in different Y bands.
...
for (let y = 0; y < img.height; y++) {
    for (let x = 0; x < img.width; x++) {
        ...
        const isGreen = (g > 35) && (g - r > 4) && (g - b > 4);
        if (isGreen) {
            const bandIdx = Math.floor((y / img.height) * 10);
            bands[bandIdx]++;
            if (greenCoords.length < 20) {
                greenCoords.push({x, y, r, g, b});
            }
        }
    }
}

console.log('Green pixels by Y band (0 top, 9 bottom):');
bands.forEach((count, i) => {
    console.log(`  Band ${i}: ${count} green pixels`);
});

console.log('First 20 green pixels coordinates:');
console.log(greenCoords);
