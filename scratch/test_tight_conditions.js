...
let whiteShirtCount = 0;
let falsePositives = 0;
let falsePositiveSamples = [];
...
for (let i = 0; i < img.pixels.length; i += 4) {
    ...
    // Standard isWhite condition
    const isWhite = (r > 140 && g > 140 && b > 140) && (Math.abs(r - g) < 15 && Math.abs(g - b) < 15 && Math.abs(r - b) < 15);
    
    if (isWhite) {
        // Check if it satisfies skin characteristics
        const hasSkinCharacteristics = (r > g) && (g > b - 15) && (r - b > 10);
        if (hasSkinCharacteristics) {
            falsePositives++;
            if (falsePositiveSamples.length < 20) {
                const idx = i / 4;
                const x = idx % img.width;
                const y = Math.floor(idx / img.width);
                falsePositiveSamples.push({ x, y, rgb: [r, g, b] });
            }
        } else {
            whiteShirtCount++;
        }
...
console.log(`Original White pixels matching isWhite: ${whiteShirtCount + falsePositives}`);
console.log(`Remaining White pixels after filtering skin characteristics: ${whiteShirtCount}`);
console.log(`Filtered out pixels (likely skin highlights or face glare): ${falsePositives}`);
console.log('Sample filtered pixels:');
falsePositiveSamples.forEach(s => {
    console.log(`Pixel at (${s.x}, ${s.y}): RGB=[${s.rgb.join(', ')}]`);
});
